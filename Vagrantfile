...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  if vagrant_config and vagrant_config['AWS-GEOEVENTS']['DEPLOY_TO_AWS']==true
    ...
    config.vm.provider :aws do |aws, override|
      ...
      aws.access_key_id = vagrant_config['AWS-GEOEVENTS']['ACCESS_KEY_ID']
      aws.secret_access_key = vagrant_config['AWS-GEOEVENTS']['SECRET_ACCESS_KEY']
      aws.keypair_name = vagrant_config['KEYPAIR_NAME']
      aws.security_groups = vagrant_config['AWS-GEOEVENTS']['SECURITY_GROUPS']
      aws.ami = vagrant_config['AWS-GEOEVENTS']['AMI']
      aws.tags = {
        'Name' => 'Geoevents',
        'Maturity' => 'Deployed'
      }
